...
namespace NeguPi {
  ...
  {
    return readBits(MPU6050_RA_MOT_DETECT_CTRL, MPU6050_DETECT_ACCEL_ON_DELAY_BIT, MPU6050_DETECT_ACCEL_ON_DELAY_LENGTH);
  }
  ...
  {
    return readBit(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_TEMP_DIS_BIT) == 0; // 1 is actually disabled here
  }
  ...
  {
    readBytes(MPU6050_RA_FIFO_COUNTH, length, data);
  }
  ...
  bool MPU6050::writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address,
  ...
  {
    // reset device
    LogD() << "Resetting MPU6050...";
    reset();

    std::this_thread::sleep_for(std::chrono::microseconds(30000)); // wait after reset
    ...
    // disable sleep mode
    LogD() << "Disabling sleep mode...";
    setSleepEnabled(false);

    // get MPU hardware revision
    LogD() << "Selecting user bank 16...";
    setMemoryBank(0x10, true, true);
    LogD() << "Selecting memory byte 6...";
    setMemoryStartAddress(0x06);
    LogD() << "Checking hardware revision...";
    uint8_t hwRevision __attribute__((__unused__)) = readMemoryByte();
    LogD() << "Revision @ user[16][6] = 0x" << std::hex << (int)hwRevision << std::dec;
    LogD() << "Resetting memory bank selection to 0...";
    setMemoryBank(0, false, false);

    // check OTP bank valid
    LogD() << "Reading OTP bank valid flag...";
    uint8_t otpValid __attribute__((__unused__)) = getOTPBankValid();
    LogD() << "OTP bank is " << (otpValid ? "valid!" : "invalid!");

    // get X/Y/Z gyro offsets
    LogD() << "Reading gyro offset values...";
    int8_t xgOffset = getXGyroOffset();
    int8_t ygOffset = getYGyroOffset();
    int8_t zgOffset = getZGyroOffset();
    LogD() << "X gyro offset = " << xgOffset;
    LogD() << "Y gyro offset = " << ygOffset;
    LogD() << "Z gyro offset = " << zgOffset;

    // setup weird slave stuff (?)
    LogD() << "Setting slave 0 address to 0x7F...";
    setSlaveAddress(0, 0x7F);
    LogD() << "Disabling I2C Master mode...";
    setI2CMasterModeEnabled(false);
    LogD() << "Setting slave 0 address to 0x68 (self)...";
    setSlaveAddress(0, 0x68);
    LogD() << "Resetting I2C Master control...";
    resetI2CMaster();

    std::this_thread::sleep_for(std::chrono::microseconds(20000));

    // load DMP code into memory banks
    LogD() << "Writing DMP code to MPU memory banks ("
           << MPU6050_DMP_CODE_SIZE
           << " bytes)";
    if (writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE)) {
      LogM() << "Success! DMP code written and verified.";

      // write DMP configuration
      LogD() << "Writing DMP configuration to MPU memory banks ("
             << MPU6050_DMP_CONFIG_SIZE
             << " bytes in config def)";
      if (writeProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE)) {
        LogM() << "Success! DMP configuration written and verified.";

        LogD() << "Setting clock source to Z Gyro...";
        setClockSource(MPU6050_CLOCK_PLL_ZGYRO);

        LogD() << "Setting DMP and FIFO_OFLOW interrupts enabled...";
        setIntEnabled(0x12);

        LogD() << "Setting sample rate to 200Hz...";
        setRate(4); // 1khz / (1 + 4) = 200 Hz

        LogD() << "Setting external frame sync to TEMP_OUT_L[0]...";
        setExternalFrameSync(MPU6050_EXT_SYNC_TEMP_OUT_L);

        LogD() << "Setting DLPF bandwidth to 42Hz...";
        setDLPFMode(MPU6050_DLPF_BW_42);

        LogD() << "Setting gyro sensitivity to +/- 2000 deg/sec...";
        setFullScaleGyroRange(MPU6050_GYRO_FS_2000);

        LogD() << "Setting DMP configuration bytes (function unknown)...";
        setDMPConfig1(0x03);
        setDMPConfig2(0x00);

        LogD() << "Clearing OTP Bank flag...";
        setOTPBankValid(false);

        LogD() << "Setting X/Y/Z gyro offsets to previous values...";
        setXGyroOffset(xgOffset);
        setYGyroOffset(ygOffset);
        setZGyroOffset(zgOffset);

        LogD() << "Setting X/Y/Z gyro user offsets to zero...";
        setXGyroOffset(0);
        setYGyroOffset(0);
        setZGyroOffset(0);

        LogD() << "Writing final memory update 1/7 (function unknown)...";
        uint8_t dmpUpdate[16], j;
        uint16_t pos = 0;
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);

        LogD() << "Writing final memory update 2/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
        ...
        LogD() << "Reading FIFO count...";
        uint8_t fifoCount = getFIFOCount();
        uint8_t fifoBuffer[128];

        LogD() << "Current FIFO count=" << fifoCount;
        getFIFOBytes(fifoBuffer, fifoCount);

        LogD() << "Setting motion detection threshold to 2...";
        setMotionDetectionThreshold(2);

        LogD() << "Setting zero-motion detection threshold to 156...";
        setZeroMotionDetectionThreshold(156);

        LogD() << "Setting motion detection duration to 80...";
        setMotionDetectionDuration(80);

        LogD() << "Setting zero-motion detection duration to 0...";
        ...
        LogD() << "Writing final memory update 3/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);

        LogD() << "Writing final memory update 4/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);

        LogD() << "Writing final memory update 5/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);

        LogM() << "Waiting for FIFO count > 2...";
        while ((fifoCount = getFIFOCount()) < 3);

        LogM() << "Current FIFO count=" << fifoCount;

        LogD() << "Reading FIFO data...";
        getFIFOBytes(fifoBuffer, fifoCount);

        LogD() << "Reading interrupt status...";
        uint8_t mpuIntStatus __attribute__((__unused__)) = getIntStatus();

        LogD() << "Current interrupt status=0x" << std::hex << mpuIntStatus << std::dec;

        LogD() << "Reading final memory update 6/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        ...
        LogD() << "Writing final memory update 7/7 (function unknown)...";
        for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
        writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);

        LogD() << "DMP is good to go! Finally.";

        LogD() << "Disabling DMP (you turn it on later)...";
        setDMPEnabled(false);

        LogD() << "Setting up internal 42-byte (default) DMP packet buffer...";
        dmpPacketSize_ = 42;
        dmpPacketBuffer_ = new uint8_t[dmpPacketSize_];

        LogD() << "Resetting FIFO and clearing INT status one last time...";
        resetFIFO();
        getIntStatus();
      } else {
        LogF() << "ERROR! DMP configuration verification failed.";
        return 2; // configuration block loading failed
      }
    } else {
      LogF() << "ERROR! DMP code verification failed.";
  ...
  VectorInt16 MPU6050::dmpGetAccel16(const uint8_t* packet)
  {
    // TODO: accommodate different arrangements of sent data (ONLY default supported now)
    if (packet == 0) packet = dmpPacketBuffer_;
    return VectorInt16((packet[28] << 8) + packet[29],
  ...
  uint8_t MPU6050::dmpProcessFIFOPacket(const unsigned char *dmpData) {
      /*for (uint8_t k = 0; k < dmpPacketSize_; k++) {
  ...
  uint8_t MPU6050::dmpReadAndProcessFIFOPacket(uint8_t numPackets, uint8_t *processed) {
      uint8_t status;
      uint8_t buf[dmpPacketSize_];
      for (uint8_t i = 0; i < numPackets; i++) {
          // read packet from FIFO
          getFIFOBytes(buf, dmpPacketSize_);
